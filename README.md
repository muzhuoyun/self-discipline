...
- **孝（10）**：关心问候父母(3) · 及时联系陪伴(3) · 体谅分担(2) · 虚心听劝(2) —— 人品与处世
- **诚（10）**：不欺骗他人(3) · 言出必行(3) · 守约守时(2) · 不自欺(2)
- **和（10）**：待人温和有礼(3) · 换位思考(3) · 不抱怨不指责(2) · 懂得感恩(2)
- **勤（10）**：尽职尽责(3) · 今日事今日毕(3) · 主动担当(2) · 不拖沓(2)

| 等级 | 分数 | 含义 |
|---|---|---|
| 🟢 精进日 | 75~100 | 状态不错，继续保持 |
| 🟡 合格日 | 50~74 | 守住了底线 |
| 🔴 调整日 | 0~49 | 明天从最弱的一项补起 |
...
### 🏠 今日
总分卡片、等级徽章、较昨日增减、连续打卡天数、十类指标星标一览。
点「💪 打卡」后 **AI 教练结合当天数据流式生成一段短评**（重新打分后可一键重新评价）。

### ✅ 打分
纯点击勾选 / 选等级，自动算分，零文字输入。
**🤖 AI 辅助判断**：输入任何话都可以——聊天、提问、或描述实际情况。AI 会**先判断你的意图**：
闲聊/提问/信息不足时直接简短回复你；描述足够时再走判断流程（先识别涉及哪些选项 →
再判断该不该勾选），自动应用并给出逐条理由。支持多轮对话，每轮都记录
（按「日期 + 条目」隔离，退出再进仍在）。

### 📅 历史（日历 + 趋势合并）
- 月历按总分上色，一眼看出整月状态；点击任意日期可查看历史与 AI 评价
